...
<table class="table table-sm mt-3 tabelas">
	<thead class="thead-light">
		<tr>
			...
			<th scope="col" class="d-none d-md-block">Descrição da Venda</th>
			<th scope="col">Valor</th>
			<th scope="col" class="d-none d-md-block">Data Da compra</th>
	...
	if($txtbuscar == '' and $cbbuscar == ''){
		$res = $pdo->query("SELECT * from cadastro order by nome asc LIMIT $limite, $itens_por_pagina");
	}else if($txtbuscar != '' and $cbbuscar == ''){
		$txtbuscar = '%'.@$_POST['txtbuscar'].'%';
		$res = $pdo->query("SELECT * from cadastro where (nome LIKE '$txtbuscar' or observacao LIKE '$txtbuscar') order by nome asc");
	}else{
		$txtbuscar = '%'.@$_POST['txtbuscar'].'%';
		$res = $pdo->query("SELECT * from cadastro where (nome LIKE '$txtbuscar' or observacao LIKE '$txtbuscar') and cadastro = '$cbbuscar' order by nome asc");
	...
	$dados = $res->fetchAll(PDO::FETCH_ASSOC);
		...
		$res_todos = $pdo->query("SELECT * from cadastro");
	...
	for ($i=0; $i < count($dados); $i++) { 
			...
			$res_cat = $pdo->query("SELECT * from cadastro where id = '$id'");
			$dados_cat = $res_cat->fetchAll(PDO::FETCH_ASSOC);
			$nome_cat = $dados_cat[0]['data_compra'];
 ...
 if ($nome == '') {
		...
		<tr>
			...
			<td class="d-none d-md-block">'.$observacao.'</td>
			<td>'.$valor.'</td>
